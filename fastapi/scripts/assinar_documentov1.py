...
async def assinar_documento(
    ...
    try:
        ...
        if not dados_assinante.get("senha"):
            output["erro"] = "Senha do assinante não encontrada no banco"
            return output
        
        if not dados_assinante.get("login_sei"):
            output["erro"] = "Login SEI do assinante não encontrado no banco"
            return output
        
        if not dados_assinante.get("nome_completo"):
            output["erro"] = "Nome completo do assinante não encontrado no banco"
            return output
        ...
        # 2) ABRE SESSÃO SEI
        async with criar_sessao_sei(chat_id=chat_id, sigla=sigla) as sessao:
            page = sessao['page']
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(description="ARGUS - Assinar Documento SEI v2.0")
    ...
    if not args.chat_id and not args.sigla:
        parser.error("Informe --chat-id ou --sigla")
